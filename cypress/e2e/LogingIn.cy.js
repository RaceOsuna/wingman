describe('template spec', () => {
  ...
  it('Should be able to view the home page', () => {
    ...
    cy.get('.form-container').within(() => {
      ...
      cy.contains('a', 'Submit').should('be.visible')
    ...
    it('Should be able to login and view a players stats', () => {
      ...
      cy.get('.player-profile').within(() => {
        ...
        cy.get('.rank-details > :nth-child(3)').should('be.visible').should('have.text', 'LP: 124148')
        cy.get('.rank-details > :nth-child(4)').should('be.visible').should('have.text', 'Total Kills: 73765')
        cy.get('.rank-details > :nth-child(5)').should('be.visible').should('have.text', 'Total Damage: 9954011')
      })
    })
})